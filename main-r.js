var reactVersions = {
  ...
  "cloud": "1.689.3",
  "components": "1.689.3",
  "componentsPreviewLayer": "1.689.3",
  "core": "1.689.3",
  "documentServices": "1.689.3",
  "editingRendererPlugins": "1.689.3",
  "fonts": "1.689.3",
  "layout": "1.689.3",
  "qaAutomation": "1.689.3",
  "skins": "1.689.3",
  "testUtils": "1.689.3",
  "tpa": "1.689.3",
  "tpaIntegration": "1.689.3",
  "tweenEngine": "1.689.3",
  "utils": "1.689.3",
  "wixSites": "1.689.3",
  "wixappsBuilder": "1.689.3",
  "wixappsClassics": "1.689.3",
  "wixappsCore": "1.689.3"
};
...
function instrument(serviceTopology, wixBiSession, queryUtil, siteModel) {
    ...
    wixBiSession.beat = function (et) {
        ...
        if (!rendererModel.previewMode) {
            ...
            var src = base + '/bt?src=29&evid=3&pn=1&vsi=' + wixBiSession.viewerSessionId +
                '&url=' + encodeURIComponent(url.substring(0, 256)) +
                '&v=' + v +
                '&et=' + et +
                '&isp=' + (isPremium ? 1 : 0) +
    ...
    var notify = function (name, code, severity) {
        notify = function () {}; // only want to report one error per session

        var src = base + '/trg?errn=' + encodeURIComponent(name) + '&errc=' + code + '&sev=' + severity +
            '&errscp=core&cat=2&iss=1&majorVer=3';
    ...
    window.onerror = function (errorMsg, url, line, column, err) {
        ...
        if (!ignoreError(where)) {
            notify('JAVASCRIPT_ERROR', 111022, 40, errorMsg, where, line, column); // JAVASCRIPT_ERROR from packages/core/src/main/bi/errors.js
        }
    ...
    if (window.console) {
        ...
        if (origError) {
            console.error = function () {
                notify.bind(null, 'CONSOLE_ERROR', 111023, 30).apply(null, arguments); // CONSOLE_ERROR from packages/core/src/main/bi/errors.js
    ...
    requirejs.onError = function (err) {
        ...
        notify('REQUIREJS_ERROR', 111024, 40, err.message, modules, where); // REQUIREJS_ERROR from packages/core/src/main/bi/errors.js
...
function render(isServerSide, isPreview, contentCache, queryUtil, siteModel, wixBiSession) {
    ...
    function load(pkgs, callback) {
        ...
        requirejs(pkgs, function () {
            ...
            if (!queryUtil.isParameterTrue('reload')) {
